...
header('Content-Disposition: attachment; filename="relatorio_fiscais_aprovados_' . date('Y-m-d_H-i-s') . '.xls"');
...
<head>
    <meta charset="UTF-8">
    <title>Relatório de Fiscais Aprovados</title>
</head>
<body>
    <table border="1">
        <tr>
        ...
        <tr>
            ...
            <th style="background-color: #2196F3; color: white;">Sala</th>
        </tr>
        ...
        <tr>
            <td colspan="2">Total de Fiscais Aprovados: <?= count($fiscais) ?></td>
            <td colspan="2">Fiscais Alocados: <?= count(array_filter($fiscais, function($f) { return $f['status_alocacao'] == 'Alocado'; })) ?></td>
            <td colspan="2">Fiscais Não Alocados: <?= count(array_filter($fiscais, function($f) { return $f['status_alocacao'] == 'Não Alocado'; })) ?></td>
        </tr>
        <tr>
            <th colspan="6" style="background-color: #f0f0f0; text-align: left;">
