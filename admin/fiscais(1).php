...
try {
    $stmt = $db->query("
        SELECT f.id, f.nome, f.email, f.celular, f.whatsapp, f.cpf, f.data_nascimento, 
               f.status, f.created_at, f.observacoes, f.concurso_id, c.orgao, c.numero_concurso, c.ano_concurso, c.cidade, c.estado,
                ...
                <div class="row">
                    ...
                    <div class="col-md-3">
                        ...
                        <select class="form-select" id="concursoFilter">
                            ...
                            <?php
                            $concursos = $db->query("SELECT id, titulo, orgao, numero_concurso, ano_concurso, cidade, estado FROM concursos WHERE status = 'ativo'")->fetchAll();
                            foreach ($concursos as $concurso) {
                                echo "<option value='{$concurso['id']}'> {$concurso['titulo']} {$concurso['numero_concurso']}/{$concurso['ano_concurso']} da {$concurso['orgao']} de {$concurso['cidade']}/{$concurso['estado']}
                                </option>";
                            }
                            ?>
                        </select>
